...
import { FaChevronLeft, FaChevronRight, FaInfoCircle, FaTrash, FaDollarSign, FaEdit, FaFileInvoice, FaTimes } from 'react-icons/fa'; // Añadido FaTimes
import AddPaymentModal from './admin/financials/AddPaymentModal';

// --- (Helpers y Constantes - Sin cambios) ---
...
function FinancialManager() {
  ...
  const [isUploading, setIsUploading] = useState(false); // Para la barra de progreso
  
  // --- NUEVOS ESTADOS PARA LA IA ---
  const [isProcessingIA, setIsProcessingIA] = useState(false); // Para el estado general de "cargando IA"
  const [iaProcessedData, setIaProcessedData] = useState(null); // Aquí se guardan los datos de la IA: { concept, amount, items }
  ...
  // --- Handlers (los de cheques, cuentas por pagar, etc. no cambian) ---
  const handleInitialBalanceSubmit = async (e) => { e.preventDefault(); /* ... (sin cambios) ... */ if (!initialBalance.amount || !initialBalance.date) { return toast.error("Complete monto y fecha."); } if (!window.confirm("¿Seguro?")) { return; } toast.loading("Registrando..."); try { const initialDate = getDateFromInput(initialBalance.date); await addDoc(collection(db, 'registrosFinancieros'), { type: 'ingreso', concept: 'Ajuste de Saldo Inicial', amount: Number(initialBalance.amount), category: 'Ajuste de Saldo', date: Timestamp.fromDate(initialDate), isManual: true, }); toast.dismiss(); toast.success("Saldo inicial registrado."); setInitialBalance({ amount: '', date: new Date().toISOString().split('T')[0] }); fetchData(); } catch (error) { toast.dismiss(); toast.error("Error al registrar el saldo."); } };
  ...
  const handleAddCheque = async (e) => { e.preventDefault(); /* ... (sin cambios) ... */ if (!newCheque.emisor || !newCheque.monto || !newCheque.fechaCobro) return toast.error("Complete todos los campos."); try { const fechaCobroDate = getDateFromInput(newCheque.fechaCobro); await addDoc(collection(db, 'pendingChecks'), { emisor: newCheque.emisor, monto: Number(newCheque.monto), fechaCobro: Timestamp.fromDate(fechaCobroDate), status: 'pendiente', createdAt: serverTimestamp() }); toast.success("Cheque agregado."); setNewCheque({ emisor: '', monto: '', fechaCobro: '' }); fetchData(); } catch (error) { toast.error("Error al agregar el cheque."); } };
  const handleAddPayable = async (e) => { e.preventDefault(); /* ... (sin cambios) ... */ const { proveedor, concepto, monto, fechaVencimiento } = newPayable; if (!proveedor || !monto || !fechaVencimiento) return toast.error("Complete Proveedor, Monto Total y Fecha."); toast.loading('Guardando cuenta...'); try { const fechaVencimientoDate = getDateFromInput(fechaVencimiento); await addDoc(collection(db, 'pendingPayables'), { proveedor: proveedor.trim(), concepto: concepto.trim(), montoTotal: Number(monto), montoPagado: 0, fechaVencimiento: Timestamp.fromDate(fechaVencimientoDate), status: 'pendiente', createdAt: serverTimestamp() }); toast.dismiss(); toast.success("Cuenta agregada."); setNewPayable({ proveedor: '', concepto: '', monto: '', fechaVencimiento: '' }); fetchData(); } catch (error) { toast.dismiss(); toast.error("Error al agregar la cuenta."); } };
  const handleEditSubmit = async (e) => { e.preventDefault(); /* ... (sin cambios) ... */ if (!editingRecord) return; toast.loading("Guardando..."); try { const { id, ...dataToUpdate } = editingRecord; dataToUpdate.date = Timestamp.fromDate(getDateFromInput(dataToUpdate.date)); dataToUpdate.amount = Number(dataToUpdate.amount); await updateDoc(doc(db, 'registrosFinancieros', id), dataToUpdate); toast.dismiss(); toast.success("Movimiento actualizado."); setIsEditModalOpen(false); setEditingRecord(null); fetchData(); } catch (error) { toast.dismiss(); toast.error("Error al guardar."); } };
  const handleStatusChange = async (cheque, newStatus) => { /* ... (sin cambios) ... */ if (newStatus === 'cobrado') { if (!window.confirm(`¿Confirmas el cobro de ${cheque.emisor}?`)) return fetchData(); toast.loading("Procesando cobro..."); try { await addDoc(collection(db, 'registrosFinancieros'), { type: 'ingreso', amount: cheque.monto, concept: `Cobro de cheque de ${cheque.emisor}`, category: 'Venta de Productos', date: serverTimestamp() }); await updateDoc(doc(db, 'pendingChecks', cheque.id), { status: 'cobrado' }); toast.dismiss(); toast.success("Cheque cobrado y registrado."); fetchData(); } catch (error) { toast.dismiss(); toast.error("Error al procesar el cobro."); } } else { try { await updateDoc(doc(db, 'pendingChecks', cheque.id), { status: newStatus }); toast.success(`Estado actualizado.`); fetchData(); } catch (error) { toast.error("Error al actualizar."); } } };
  const handlePayableStatusChange = async (payable, newStatus) => { /* ... (sin cambios) ... */ if (newStatus === 'pagado') { const montoTotal = payable.montoTotal !== undefined ? payable.montoTotal : (payable.monto || 0); const montoPagado = payable.montoPagado || 0; const montoRestante = montoTotal - montoPagado; if (!window.confirm(`¿Confirmas marcar esta cuenta como Pagada? Se registrará un egreso por el total restante de ${formatCurrency(montoRestante)}.`)) { return fetchData(); } if (montoRestante < 0) { toast.error("El monto pagado ya supera el total. No se puede registrar más."); return fetchData(); } toast.loading("Procesando pago total..."); try { const payableRef = doc(db, 'pendingPayables', payable.id); if (montoRestante > 0) { await addDoc(collection(db, 'registrosFinancieros'), { type: 'egreso', amount: montoRestante, concept: `Pago (saldo) a ${payable.proveedor}: ${payable.concepto || 'S/C'}`, category: 'Otros Gastos', date: serverTimestamp(), isManual: false, linkedPayableId: payable.id }); } await updateDoc(payableRef, { status: 'pagado', montoPagado: montoTotal }); toast.dismiss(); toast.success("¡Pago total registrado y cuenta cerrada!"); fetchData(); } catch (error) { toast.dismiss(); toast.error("Error al procesar el pago total."); console.error(error); } } };
  const handleDeleteCheque = async (chequeId, emisor) => { /* ... (sin cambios) ... */ if (!window.confirm(`¿Eliminar cheque de "${emisor}"?`)) return; toast.loading('Eliminando...'); try { await deleteDoc(doc(db, "pendingChecks", chequeId)); toast.dismiss(); toast.success("Cheque eliminado."); fetchData(); } catch (error) { toast.dismiss(); toast.error("Error al eliminar."); } };
  const handleDeletePayable = async (payableId, proveedor) => { /* ... (sin cambios) ... */ if (!window.confirm(`¿Eliminar cuenta de "${proveedor}"?`)) return; toast.loading('Eliminando...'); try { await deleteDoc(doc(db, "pendingPayables", payableId)); toast.dismiss(); toast.success("Cuenta eliminada."); fetchData(); } catch (error) { toast.dismiss(); toast.error("Error al eliminar."); } };
  const openEditModal = (record) => { /* ... (sin cambios) ... */ const currentCategories = record.type === 'ingreso' ? INCOME_CATEGORIES : EXPENSE_CATEGORIES; const recordDate = record.date.toDate(); const formattedDate = recordDate.toISOString().split('T')[0]; const recordToEdit = { ...record, category: record.category || currentCategories[0], date: formattedDate }; setEditingRecord(recordToEdit); setIsEditModalOpen(true); };
  const openPaymentModal = (payable) => { /* ... (sin cambios) ... */ setPayableToPay(payable); setIsPaymentModalOpen(true); };
  const closePaymentModal = () => { /* ... (sin cambios) ... */ setPayableToPay(null); setIsPaymentModalOpen(false); };
  const handlePaymentDone = () => { /* ... (sin cambios) ... */ closePaymentModal(); fetchData(); };
  const openPayableEditModal = (payable) => { /* ... (sin cambios) ... */ let formattedDate = ''; if (payable.fechaVencimiento && payable.fechaVencimiento instanceof Timestamp) { formattedDate = payable.fechaVencimiento.toDate().toISOString().split('T')[0]; } setPayableToEdit({ ...payable, montoTotal: payable.montoTotal !== undefined ? payable.montoTotal : (payable.monto || 0), fechaVencimiento: formattedDate }); setIsPayableEditModalOpen(true); };
  const closePayableEditModal = () => { /* ... (sin cambios) ... */ setIsPayableEditModalOpen(false); setPayableToEdit(null); };
  const handlePayableEditingChange = (e) => { /* ... (sin cambios) ... */ const { name, value } = e.target; setPayableToEdit(prev => ({ ...prev, [name]: value })); };
  const handlePayableEditSubmit = async (e) => { /* ... (sin cambios) ... */ e.preventDefault(); if (!payableToEdit) return; const { id, proveedor, concepto, montoTotal, fechaVencimiento } = payableToEdit; if (!proveedor || !montoTotal || !fechaVencimiento) { return toast.error("Proveedor, Monto Total y Fecha son obligatorios."); } toast.loading('Actualizando cuenta...'); try { const payableRef = doc(db, 'pendingPayables', id); await updateDoc(payableRef, { proveedor: proveedor.trim(), concepto: concepto.trim(), montoTotal: Number(montoTotal), fechaVencimiento: Timestamp.fromDate(getDateFromInput(fechaVencimiento)) }); toast.dismiss(); toast.success('¡Cuenta actualizada!'); closePayableEditModal(); fetchData(); } catch (error) { toast.dismiss(); toast.error('Error al actualizar la cuenta.'); console.error("Error updating payable:", error); } };
  // --- Fin Handlers ---


  // --- NUEVO: Handlers del Formulario Manual (Modificados para IA) ---

  // El input del formulario cambia el estado 'manualRecord'
  const handleRecordInputChange = (e) => {
    const { name, value } = e.target;
    setManualRecord(prev => ({ ...prev, [name]: value }));
  };

  // El input de archivo cambia el estado 'receiptFile'
  ...
  // --- NUEVO: Lógica Centralizada de guardado y procesado de IA ---
  const handleRecordSubmit = async (e) => {
    e.preventDefault();
    if (isProcessingIA || isUploading) return; // Evita doble click

    // --- FLUJO 2: Confirmar y Guardar (Datos de IA ya están cargados) ---
    if (iaProcessedData) {
      toast.loading("Guardando registro...");
      try {
        const docData = {
          type: manualRecord.type,
          concept: manualRecord.concept, // Concepto general (de la IA)
          amount: Number(manualRecord.amount), // Monto total (de la IA)
          category: manualRecord.category,
          date: serverTimestamp(),
          isManual: false, // Fue asistido por IA
          receiptURL: receiptURL, // URL de la imagen
          iaItems: iaProcessedData.items || [] // Lista de ítems de la IA
        };

        await addDoc(collection(db, 'registrosFinancieros'), docData);
        toast.dismiss();
        toast.success("Movimiento registrado con IA.");
        resetManualRecordForm();
        fetchData();
      } catch (error) {
        toast.dismiss();
        toast.error("Error al guardar el registro.");
        console.error("Error saving IA record:", error);
      }
      return;
    }

    // --- FLUJO 1: Procesar con IA (Hay archivo pero no datos de IA) ---
    if (receiptFile && !iaProcessedData) {
      setIsProcessingIA(true);
      setIsUploading(true); // Muestra barra de progreso
      ...
      uploadTask.on('state_changed',
        ...
        async () => {
          // Subida completada
          toast.dismiss(uploadToastId);
          toast.loading("Imagen subida. Procesando con IA...");
          setIsUploading(false); // Termina la subida, empieza la IA

          const downloadURL = await getDownloadURL(uploadTask.snapshot.ref);
          setReceiptURL(downloadURL); // Guardamos la URL para el guardado final
          ...
          try {
            ...
            setManualRecord(prev => ({
              ...prev,
              concept: data.concept || prev.concept, // Rellena el concepto
              amount: data.amount || prev.amount     // Rellena el monto
            }));

            toast.dismiss();
            toast.success("Datos procesados. Por favor, revisa y confirma.");
          
          } catch (error) {
            console.error("Error al procesar con IA:", error);
            toast.dismiss();
            toast.error("Error de IA: " + error.message);
            // Aunque la IA falle, permitimos el guardado manual
            // Por eso no reseteamos receiptURL
          } finally {
            setIsProcessingIA(false); // Termina el procesamiento de IA
          }
        }
      );
      return;
    ...
    // --- FLUJO 3: Guardado Manual (No hay archivo) ---
    if (!receiptFile) {
      if (!manualRecord.concept || !manualRecord.amount) {
        return toast.error("Complete Concepto y Monto.");
      }
      toast.loading("Registrando manualmente...");
      try {
  ...
  // --- NUEVO: Lógica del botón de guardado ---
  ...
  // --- NUEVO: Monto total calculado si hay datos de IA ---
  const totalAmountFromIA = useMemo(() => {
    if (iaProcessedData && iaProcessedData.items) {
      ...
      // Actualiza el estado del formulario automáticamente
