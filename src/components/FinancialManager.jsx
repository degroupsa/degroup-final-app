...
import { FaChevronLeft, FaChevronRight, FaInfoCircle, FaTrash, FaDollarSign, FaEdit } from 'react-icons/fa';
import AddPaymentModal from './admin/financials/AddPaymentModal'; // Ajusta esta ruta si es necesario
...
function FinancialManager() {
  ...
  const [financialRecords, setFinancialRecords] = useState([]); // Inicializado como array vacío
  ...
  const handleRecordSubmit = async (e) => { e.preventDefault(); if (!manualRecord.concept || !manualRecord.amount || !manualRecord.category) return toast.error("Complete todos los campos."); toast.loading("Registrando..."); try { await addDoc(collection(db, 'registrosFinancieros'), { type: manualRecord.type, concept: manualRecord.concept, amount: Number(manualRecord.amount), category: manualRecord.category, date: serverTimestamp(), isManual: true }); toast.dismiss(); toast.success("Movimiento registrado."); setManualRecord({ type: 'ingreso', concept: '', amount: '', category: INCOME_CATEGORIES[0] }); fetchData(); } catch (error) { toast.dismiss(); toast.error("Error al registrar."); } };
  ...
  const handlePayableStatusChange = async (payable, newStatus) => {
    if (newStatus === 'pagado') {
      ...
      if (!window.confirm(`¿Confirmas marcar esta cuenta como Pagada? Se registrará un egreso por el total restante de ${formatCurrency(montoRestante)}.`)) {
        return fetchData(); // Refresca para resetear el dropdown
      }
      if (montoRestante < 0) {
      ...
      try {
        ...
        if (montoRestante > 0) {
          await addDoc(collection(db, 'registrosFinancieros'), {
            type: 'egreso', amount: montoRestante,
            concept: `Pago (saldo) a ${payable.proveedor}: ${payable.concepto || 'S/C'}`,
            category: 'Otros Gastos', date: serverTimestamp(), isManual: false,
        ...
        fetchData();
      } catch (error) { toast.dismiss(); toast.error("Error al procesar el pago total."); console.error(error); }
    }
  };
  ...
  // Handlers Modal Edición Cuenta por Pagar
  ...
  return (
    <>
      ...
      {isPayableEditModalOpen && payableToEdit && (
        <div className={styles.editModalOverlay}>
          <div className={styles.editModalContent} style={{ maxWidth: '600px' }}>
            <h2>Editar Cuenta por Pagar</h2>
            <form onSubmit={handlePayableEditSubmit}>
              <div className={styles.formGrid}>
                <div className={styles.formGroup}><label>Proveedor</label><input type="text" name="proveedor" value={payableToEdit.proveedor} onChange={handlePayableEditingChange} required /></div>
                <div className={styles.formGroup}><label>Concepto (Opcional)</label><input type="text" name="concepto" value={payableToEdit.concepto} onChange={handlePayableEditingChange} /></div>
                <div className={styles.formGroup}><label>Monto Total</label><input type="number" step="0.01" name="montoTotal" value={payableToEdit.montoTotal} onChange={handlePayableEditingChange} required /></div>
                <div className={styles.formGroup}><label>Fecha de Vencimiento</label><input type="date" name="fechaVencimiento" value={payableToEdit.fechaVencimiento} onChange={handlePayableEditingChange} required /></div>
              </div>
              <div className={styles.editModalActions}>
                <button type="button" onClick={closePayableEditModal}>Cancelar</button>
