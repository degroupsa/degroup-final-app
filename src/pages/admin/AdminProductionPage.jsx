...
// --- Importamos QRCodeModal y FaQrcode ---
...
const AdminProductionPage = () => {
  ...
  const fetchAllData = useCallback(async () => {
    ...
    } catch (error) { toast.error("Error al cargar datos."); console.error(error); }
  ...
  const filteredOrders = useMemo(() => {
    const lowerSearch = searchTerm.toLowerCase().trim(); // Añadido trim()
    if (!lowerSearch) return orders;
    return orders.filter(order => {
      const productName = order.productName || '';
      const clientName = order.linkedClientName || '';
      const trackingCode = order.trackingCode || '';
      return ( productName.toLowerCase().includes(lowerSearch) || clientName.toLowerCase().includes(lowerSearch) || trackingCode.toLowerCase().includes(lowerSearch) );
    });
  }, [orders, searchTerm]);
  ...
  const advanceStatus = async (order) => { /* ... (sin cambios) ... */ };
  ...
  // --- Función para Renderizar Tarjeta (AHORA INCLUYE BOTÓN QR) ---
    ...
    return (
      <div key={order.id} className={`${styles.orderCard} ${isCompleted ? styles.completedCard : ''}`}>
        ...
        <div className={styles.orderCardInfo}>
          {/* --- Contenedor y Botón QR --- */}
          <div className={styles.trackingInfo}>
             <span className={styles.trackingCode}>{order.trackingCode}</span>
             {order.trackingCode && ( // Solo mostrar si hay código
                <button
  ...
  // --- FIN Función Renderizar Tarjeta ---
