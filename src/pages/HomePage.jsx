...
// Importamos el nuevo CSS de móvil (asegúrate de crear este archivo después)
...
const quillModules = {
  toolbar: [['bold', 'italic', 'underline'], [{ 'color': [] }, { 'background': [] }], [{ 'header': [1, 2, 3, false] }], [{ 'list': 'ordered'}, { 'list': 'bullet' }], [{ 'align': [] }], ['clean']],
};
...
const HeroSlidesEditorModal = ({ slides, onSave, onClose }) => {
  ...
  const handleStartEdit = (index) => {
    if (index >= 0) {
      ...
      setFormTitle(slide.title || ''); setFormIndicatorTitle(slide.indicatorTitle || ''); setFormText(slide.text || ''); setFormButtonText(slide.buttonText || ''); setFormType(slide.type || 'image'); setMediaPreview(slide.url || ''); setEditingIndex(index);
    } else {
      setFormTitle(''); setFormIndicatorTitle(''); setFormText(''); setFormButtonText('VER MÁS'); setFormType('image'); setMediaPreview(''); setEditingIndex('NEW');
    }
    setMediaFile(null);
  };
  ...
  const handleFileChange = (e) => {
    if (e.target.files[0]) {
      setMediaFile(e.target.files[0]); setMediaPreview(URL.createObjectURL(e.target.files[0]));
    ...
    <div className={styles.adminEditorOverlay}><div className={styles.adminEditorModal}>
      ...
      {editingIndex === null ? (
        <>
          ...
          <div className={styles.formActions}><button onClick={onClose}>Cerrar</button><button onClick={async()=>{setLoading(true); await onSave('hero',{heroSlides:localSlides}); setLoading(false); onClose();}}>Guardar Todo</button></div>
        </>
      ) : (
...
const AdminSectionEditor = ({ sectionName, initialData, onSave, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if(file) { const r = ref(storage, `sections/${Date.now()}`); await uploadBytes(r, file); img = await getDownloadURL(r); }
  ...
  return (
    <div className={styles.adminEditorOverlay}><div className={styles.adminEditorModal}>
      ...
      <form onSubmit={handleSubmit} className={styles.formFields}>
        ...
        <div className={styles.formGroup}><label>Botón:</label><input value={buttonText} onChange={e=>setButtonText(e.target.value)}/></div>
        ...
        <div className={styles.formGroup}><label>Imagen:</label><input type="file" onChange={e=>{setFile(e.target.files[0]); setPrev(URL.createObjectURL(e.target.files[0]))}}/>{prev && <img src={prev} style={{width:100}}/>}</div>
...
const NewsEditModal = ({ product, currentOrder, totalItems, onSave, onClose }) => {
  const [title, setTitle] = useState(product.name||''); const [desc, setDesc] = useState(product.description||'');
  const [order, setOrder] = useState(currentOrder); const [file, setFile] = useState(null); const [loading, setLoading] = useState(false);
  return (
    <div className={styles.adminEditorOverlay}><div className={styles.adminEditorModal}>
      <h3>Editar Tarjeta</h3>
      <form onSubmit={async (e)=>{e.preventDefault(); setLoading(true); await onSave(product.id, {name:title, description:desc, order:parseInt(order)}, file); onClose();}}>
        <div className={styles.formGroup}><label>Título:</label><ReactQuill value={title} onChange={setTitle} modules={titleModules}/></div>
        <div className={styles.formGroup}><label>Desc:</label><ReactQuill value={desc} onChange={setDesc} modules={quillModules}/></div>
        <div className={styles.formGroup}><label>Posición:</label><input type="number" value={order} onChange={e=>setOrder(e.target.value)}/></div>
...
const FeaturedProductsEditorModal = ({ sectionName, initialProductIDs, onSave, onClose, allProducts }) => {
  ...
  const [isNew, setIsNew] = useState(false); const [nTitle, setNTitle] = useState(''); const [nDesc, setNDesc] = useState(''); const [nFile, setNFile] = useState(null);
  ...
  const handleCreate = async (e) => {
      ...
      const pg = await addDoc(collection(db,'marketing_pages'), {heroTitle:nTitle, heroSubtitle:truncateText(nDesc,50), heroImageUrl:url, type:'custom'});
      const pr = await addDoc(collection(db,'products'), {name:nTitle, description:nDesc, imageUrls:[url], isMarketing:true, marketingPageId:pg.id, order:999});
      setIds(prev=>new Set(prev).add(pr.id)); setIsNew(false); toast.success("Creado");
  };

  return (
    <div className={styles.adminEditorOverlay}><div className={styles.adminEditorModal}>
      <h3>Administrar {sectionName}</h3>
      {!isNew ? (
         <>
           <button className={styles.addCategoryButton} onClick={()=>setIsNew(true)}>+ Crear Noticia</button>
           ...
           <div className={styles.productListContainer}>{allProducts.filter(p=>p.name.toLowerCase().includes(search.toLowerCase())).map(p=>(
               <label key={p.id} className={styles.productListItem}><input type="checkbox" checked={ids.has(p.id)} onChange={()=>{const n=new Set(ids); if(n.has(p.id))n.delete(p.id); else n.add(p.id); setIds(n)}}/> {p.name}</label>
           ))}</div>
         ...
         <form onSubmit={handleCreate}>
            <div className={styles.formGroup}><label>Título:</label><ReactQuill value={nTitle} onChange={setNTitle} modules={titleModules}/></div>
            <div className={styles.formGroup}><label>Texto:</label><ReactQuill value={nDesc} onChange={setNDesc} modules={quillModules}/></div>
            <div className={styles.formGroup}><label>Imagen:</label><input type="file" required onChange={e=>setNFile(e.target.files[0])}/></div>
...
const CategoryEditorModal = ({ initialData, onSave, onClose, productCategories }) => {
    const [name, setName] = useState(initialData.name||''); const [order, setOrder] = useState(initialData.order||0);
    const [file, setFile] = useState(null); const [cat, setCat] = useState(''); const [newCat, setNewCat] = useState('');
    const handleSubmit = async (e) => {
        e.preventDefault();
        ...
        await onSave({name, order, iconUrl:url, link}, initialData.id); onClose();
    };
    return (
        <div className={styles.adminEditorOverlay}><div className={styles.adminEditorModal}>
            <h3>Categoría</h3>
            <form onSubmit={handleSubmit}>
                <div className={styles.formGroup}><label>Nombre:</label><input value={name} onChange={e=>setName(e.target.value)} required/></div>
                <div className={styles.formGroup}><label>Vincular:</label><select onChange={e=>setCat(e.target.value)}><option value="">Seleccionar</option>{productCategories.map(c=><option value={c}>{c}</option>)}<option value="__NEW__">Nueva</option></select></div>
                ...
                <div className={styles.formGroup}><label>Orden:</label><input type="number" value={order} onChange={e=>setOrder(e.target.value)}/></div>
...
function HomePage() {
  ...
  const [heroData, setHeroData] = useState({heroSlides:[{title:'DE Group',text:'',buttonText:'VER',type:'image',url:''}]});
  const [agritechnicaData, setAgritechnicaData] = useState({title:'AGRO',text:'',buttonText:'VER',imageUrl:''});
  const [finalCtaData, setFinalCtaData] = useState({title:'Final CTA',text:'',buttonText:'Contactar',imageUrl:''});
  ...
  const handleVideoEnd = () => { 
      if(heroData.heroSlides.length===1 && videoRef.current) { videoRef.current.currentTime=0; videoRef.current.play(); setProgress(0); }
      else nextSlide();
  };
  useEffect(() => {
  ...
  return (
    <div className={styles.homePage}>
      ...
      <section className={styles.heroSection}>
         ...
         {heroData.heroSlides?.map((s,i)=>(
             <div key={i} className={`${styles.heroSlide} ${i===currSlide?styles.active:''}`}>
                 {s.type==='video' ? <video src={s.url} className={styles.heroMedia} muted autoPlay={i===currSlide} playsInline ref={i===currSlide?videoRef:null} onTimeUpdate={e=>e.target.duration && setProgress((e.target.currentTime/e.target.duration)*100)} onEnded={handleVideoEnd}/> 
         ...
         <div className={styles.heroIndicators}><div className={styles.indicatorContainer}>{heroData.heroSlides?.map((s,i)=>(
             <div key={i} className={`${styles.indicatorItem} ${i===currSlide?styles.active:''}`} onClick={()=>{setCurrSlide(i); setProgress(0);}}>
                 ...
                 <span className={styles.indicatorLabel}>{s.indicatorTitle || stripHtml(s.title).substring(0,30)}</span>
             </div>
         ))}</div></div>
      </section>
              ...
              <div className={styles.carouselContainer}>
                  ...
                  <div className={styles.newsGrid} ref={newsRef}>{featuredProducts.map((p,i)=>(
                      <div key={p.id} className={styles.newsCard}>
                          {isAdmin && <div className={styles.adminCardButtons} style={{top:10,right:10,position:'absolute'}}><button className={styles.cardEditButton} onClick={()=>{setCardEdit({...p,idx:i+1}); setEditCtx('news'); toggleModal('card',true)}}><FaEdit/></button></div>}
          ...
          <div className={styles.newsContentWrapper}>
              ...
              <div className={styles.carouselContainer}>
                  ...
                  <div className={styles.newsGrid} ref={promoRef}>{promotionsList.map((p,i)=>(
                      <div key={p.id} className={styles.newsCard}>
                          {isAdmin && <div className={styles.adminCardButtons} style={{top:10,right:10,position:'absolute'}}><button className={styles.cardEditButton} onClick={()=>{setCardEdit({...p,idx:i+1}); setEditCtx('promotions'); toggleModal('card',true)}}><FaEdit/></button></div>}
                          <div className={styles.newsImage}><img src={p.imageUrls?.[0]}/></div>
                          <div className={styles.newsContent}><h3 className={styles.newsTitle} dangerouslySetInnerHTML={{__html:p.name}}/><div className={styles.newsDescription} dangerouslySetInnerHTML={{__html:truncateText(p.description,100)}}/><Link to={p.isMarketing?`/novedades/${p.marketingPageId}`:`/producto/${p.id}`} className={styles.newsLink}>LEER MÁS <FaChevronRight/></Link></div>
                      </div>
                  ))}</div>
      ...
      {modals.card && cardEdit && <NewsEditModal product={cardEdit} currentOrder={cardEdit.idx} totalItems={editCtx==='news'?featuredProducts.length:promotionsList.length} onSave={async(id, d, f)=>{
          let img = null; if(f){const r=ref(storage,`marketing/${Date.now()}`); await uploadBytes(r,f); img=await getDownloadURL(r);}
          await updateDoc(doc(db,'products',id), {name:d.name, description:d.description, order:d.order, ...(img && {imageUrls:[img]})});
          const sec = editCtx==='news'?newsData:promotionsData;
          if(d.order!==cardEdit.idx){
              const ids = [...sec.productIDs].filter(x=>x!==id); ids.splice(Math.max(0,Math.min(d.order-1,ids.length)),0,id);
              await saveData(editCtx, {productIDs:ids});
          } else { if(editCtx==='news') loadProds(sec.productIDs, setFeaturedProducts); else loadProds(sec.productIDs, setPromotionsList); }
