...
import { FaCamera, FaGlobe, FaEdit, FaUserPlus, FaUserCheck, FaPaperPlane, FaBoxOpen, FaShieldAlt, FaImages } from 'react-icons/fa';
...
function ProfilePageMobile() {
  ...
  const handleFollow = async () => {
    ...
    try {
      ...
      toast.success(
        isFollowing
          ? `Dejaste de seguir a ${profileUser.displayName}`
          : `Ahora sigues a ${profileUser.displayName}`
      );
    } catch (error) {
  ...
  const handleSaveProfile = async (updatedData) => {
    ...
    const dataToSave = {
      ...updatedData,
      displayName_lowercase: updatedData.displayName.toLowerCase(),
    };
  ...
  const handleImageUpload = async (event, type) => {
    ...
    const filePath =
      type === "avatar"
        ...
        : `cover_pictures/${currentUser.uid}`;
    ...
    try {
      ...
      const userDocRef = doc(db, "users", currentUser.uid);
      const fieldToUpdate =
        type === "avatar" ? "profileImageUrl" : "coverImageUrl";
  ...
  return (
    <div className={styles.profilePageMobile}>
      <div className={styles.profileHeader} style={{ backgroundImage: `url(${profileUser.coverImageUrl || ''})` }}>
        {isOwner && (
            <>
                <label htmlFor="coverUpload" className={styles.coverEditButton}><FaCamera /> Cambiar</label>
      ...
      <div className={styles.stackedInfo}>
        ...
        <div className={styles.nameAndRoleContainer}>
          ...
          <span className={`${styles.roleTag} ${getRoleClass(profileUser.role)}`}>
            {capitalize(profileUser.role)}
          </span>
        </div>
      ...
      <nav className={styles.contentNav}>
        <button onClick={() => setActiveView('posts')} className={activeView === 'posts' ? styles.active : ''}>
          <FaImages/> Publicaciones
        </button>
        {isOwner && (
          <>
            <button onClick={() => setActiveView('orders')} className={activeView === 'orders' ? styles.active : ''}>
              <FaBoxOpen/> Pedidos
            </button>
            <button onClick={() => setActiveView('security')} className={activeView === 'security' ? styles.active : ''}>
